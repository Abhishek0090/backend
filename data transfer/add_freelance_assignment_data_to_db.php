<?php
    ...
    if(file_exists($filename))
    {
        ...
        for($i = 0; $i < $number_of_arrays; $i++)
        {
            ...
            $insert_assignment_sql = "INSERT INTO `assignment_freelance` (`user_id`, `stream`, `title`, `description`, `course`, `level`, `type`, `subject_tags`, `deadline`, `budget`, `files`, `submission_date`, `posted`, `under_process`, `assigned_to_pm`, `assigned_to_freelancer`, `completed`, `review_recieved`, `lost`) 
            VALUES ('$old_assignment_user_id', '$stream', '$title', '$description', '$course', '$level', '$type', '$subject_tags', '$deadline', '$budget', '$file', '$now', '$posted', '$under_process', '$assigned_to_pm', '$assigned_to_freelancer', '$completed', '$review_received', '$lost')";
            // $insert_assignment_sql = str_replace("'", "\'", $insert_assignment_sql);
