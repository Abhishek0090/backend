<?php
    ...
    $get_messages_sql = "SELECT * FROM `team_and_student_chat_assignment` WHERE `chat_id` = '$chat_id' ORDER BY `id` ASC";
    $get_messages = mysqli_query($conn, $get_messages_sql);
    while ($get_messages_data = mysqli_fetch_assoc($get_messages))
    {
    ...
    echo json_encode($messages);
    // echo json_encode($get_messages_sql);
