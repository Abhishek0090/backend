<?php
    ...
    if ($submit == "submit")
    {
        ...
        // if domain is plag check then insert into assignment_plag_check table
        // get back id using email id
        // move files using the same id

        if($domain == "Plag Check")
        {
            $insert_into_plag_check_sql = "INSERT INTO `assignment_plag_check` 
            (`name`, `country_name`, `country_code`, `number`, `email`, `files`, `submission_date`) 
            VALUES ('$name', '$country_name', '$country_code', '$number', '$email', '$assignment_files_string', '$now')";
            // echo json_encode($insert_into_plag_check_sql);
            $insert_into_plag_check_result = mysqli_query($conn, $insert_into_plag_check_sql);
        
            $get_assignment_id = "SELECT `id` FROM `assignment_plag_check` WHERE `email` = '$email' ORDER BY `id` DESC LIMIT 1";
            ...
            if($insert_into_plag_check_result == true)
            {
                $dummy_array = array(
    ...
    function mailer1($assignment_id, $user_id, $email, $firstname, $lastname, $title, $description, $type, $level, $deadline, $budget)
    {
        $to = $email;
