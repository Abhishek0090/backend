<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $art_and_craft_assignment=$_FILES['art_and_craft_assignment']['name'];

        $error = $_FILES['art_and_craft_assignment']['error'];
        
        $FileType = strtolower(pathinfo($art_and_craft_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($art_and_craft_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($art_and_craft_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$art_and_craft_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($art_and_craft_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['art_and_craft_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $profiletargetadmin = "../../files/uploads/assignments/art_and_craft/".$random_number.$_FILES['art_and_craft_assignment']['name'][$i];
        ...
        VALUES ('art_and_craft_assignment', '$arr', '$now')";
        ...
        echo json_encode(array(
            // 'received data' => $data,
            'file_name' => $art_and_craft_assignment,
