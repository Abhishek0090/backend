<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $freelancing_assignment=$_FILES['freelancing_assignment']['name'];

        $error = $_FILES['freelancing_assignment']['error'];
        
        $FileType = strtolower(pathinfo($freelancing_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($freelancing_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($freelancing_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$freelancing_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($freelancing_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['freelancing_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $old_name = $_FILES['freelancing_assignment']['name'][$i];
                $new_name = str_replace("'", "", $old_name);
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/assignments/freelancing/".$random_number.$new_name;
        ...
        VALUES ('freelancing_assignment', '$arr', '$now')";
        ...
        echo json_encode(array(
            // 'received data' => $data,
            'file_name' => $freelancing_assignment,
            ...
            'name' => $_FILES['freelancing_assignment']['name'][0],
        ));
    }
