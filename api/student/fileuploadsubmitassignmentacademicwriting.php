<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $academic_assignment=$_FILES['academic_assignment']['name'];

        $error = $_FILES['academic_assignment']['error'];
        
        $FileType = strtolower(pathinfo($academic_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($academic_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($academic_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$academic_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($academic_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['academic_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $old_name = $_FILES['academic_assignment']['name'][$i];
                $new_name = str_replace("'", "", $old_name);
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/assignments/academic_writing/".$random_number.$new_name;
        ...
        VALUES ('academic_assignment', '$arr', '$now')";
        ...
        echo json_encode(array(
            ...
            'name' => $_FILES['academic_assignment']['name'][0],
        ));
    }
