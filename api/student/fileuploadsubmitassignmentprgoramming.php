<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $programming_assignment=$_FILES['programming_assignment']['name'];

        $error = $_FILES['programming_assignment']['error'];
        
        $FileType = strtolower(pathinfo($programming_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($programming_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($programming_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$programming_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($programming_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['programming_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $old_name = $_FILES['programming_assignment']['name'][$i];
                $new_name = str_replace("'", "", $old_name);
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/assignments/programming/".$random_number.$new_name;
        ...
        VALUES ('programming_assignment', '$arr', '$now')";
        ...
        echo json_encode(array(
            // 'received data' => $data,
            'file_name' => $programming_assignment,
            ...
            'name' => $_FILES['programming_assignment']['name'][0],
        ));
    }
