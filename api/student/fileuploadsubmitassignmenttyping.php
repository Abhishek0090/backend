<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $typing_assignment=$_FILES['typing_assignment']['name'];

        $error = $_FILES['typing_assignment']['error'];
        
        $FileType = strtolower(pathinfo($typing_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($typing_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($typing_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$typing_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($typing_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['typing_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $profiletargetadmin = "../../files/uploads/assignments/typing/".$random_number.$_FILES['typing_assignment']['name'][$i];
        ...
        VALUES ('typing_assignment', '$arr', '$now')";
