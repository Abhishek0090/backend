<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $ed_assignment=$_FILES['ed_assignment']['name'];

        $error = $_FILES['ed_assignment']['error'];
        
        $FileType = strtolower(pathinfo($ed_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($ed_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($ed_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$ed_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($ed_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['ed_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $profiletargetadmin = "../../files/uploads/assignments/ed/".$random_number.$_FILES['ed_assignment']['name'][$i];
