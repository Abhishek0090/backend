<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $professional_assignment=$_FILES['professional_assignment']['name'];

        $error = $_FILES['professional_assignment']['error'];
        
        $FileType = strtolower(pathinfo($professional_assignment,PATHINFO_EXTENSION));

        for($j=0;$j<count($professional_assignment);$j++)
        {
            $FileType = strtolower(pathinfo($professional_assignment[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$professional_assignment[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($professional_assignment);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['professional_assignment']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $old_name = $_FILES['professional_assignment']['name'][$i];
                $new_name = str_replace("'", "", $old_name);
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/assignments/professional_writing/".$random_number.$new_name;
        ...
        VALUES ('professional_assignment', '$arr', '$now')";
        ...
        echo json_encode(array(
            // 'received data' => $data,
            'file_name' => $professional_assignment,
            ...
            'name' => $_FILES['professional_assignment']['name'][0],
        ));
    }
