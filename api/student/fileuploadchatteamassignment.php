    ...
    if (isset($_POST['submit']))
    {
        ...
        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            ...
            if ($profiletarget != "")
            {
                ...
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/chat/team_student_assignment/".$random_number.$new_name;
        ...
        $insert_file_data_into_sql = "INSERT INTO `uploaded_files_user_chat`(`student_id`, `file_source`, `file_name`, `file_upload_date_time`)
        VALUES ('$student_id', 'chat_file', '$arr', '$now')";

        $insert_file_data = mysqli_query($conn, $insert_file_data_into_sql);

        $get_file_id  = "SELECT `id` FROM `uploaded_files_user_chat` WHERE `file_name` = '$arr' AND `file_upload_date_time` = '$now'";
