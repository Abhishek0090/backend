<?php
    ...
    $number = $data['number'];
    $city = $data['city'];
    $role = $data['role']; // hr/ca/bd/pm/ml dev
    $reason = $data['reason'];
    $submit = $data['submit'];
    ...
    if($submit === "submit")
    {
        ...
        $insert_intern_sql = "INSERT INTO 
        `intern_applications`(`name`, `email`, `country_name`, `country_code`, `number`, `city`, `role`, `reason`, `resume`, `applied_on`) 
        VALUES ('$name', '$email', '$country_name', '$country_code', '$number', '$city', '$role', '$reason', '$resume_files_string', '$now')";
        $insert_intern = mysqli_query($conn, $insert_intern_sql);

        $get_intern_id = "SELECT * FROM `intern_applications` WHERE `email` = '$email' AND `applied_on` = '$now'";
        $get_intern_id_query = mysqli_query($conn, $get_intern_id);
        $get_intern_id_result = mysqli_fetch_assoc($get_intern_id_query);
        $intern_id = $get_intern_id_result['id'];

        if($insert_intern == true)
        {
            $resume_files = explode('_$_', $resume_files_string);
            $number_of_files = count($resume_files);
            rename("../../files/uploads/interns/resume/".$resume_files[0], "../../files/interns/resume/".$intern_id."_".$resume_files[0]);
            copy("../../files/interns/resume/".$intern_id."_".$resume_files[0], "../team/interns/resume/".$intern_id."_".$resume_files[0]);
            
            echo json_encode(array(
                'message' => 'Registration successful, we will contact you soon',
                // 'sql' => $insert_intern_sql,
                'status' => 200
            ));
            // mailer1($freelancer_id, $freelancer_email, $freelancer_firstname, $freelancer_lastname);
            exit();
        }
        else if($insert_intern == false)
        {
            echo json_encode(array(
