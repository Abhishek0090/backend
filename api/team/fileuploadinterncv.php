<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $FileType = strtolower(pathinfo($cv,PATHINFO_EXTENSION));

        for($j=0;$j<count($cv);$j++)
        {
            $FileType = strtolower(pathinfo($cv[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$cv[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($cv);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['cv']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $old_name = $_FILES['cv']['name'][$i];
                $new_name = str_replace("'", "", $old_name);
                $arr = str_replace("'", "", $arr);
                $profiletargetadmin = "../../files/uploads/interns/resume/".$random_number.$new_name;
