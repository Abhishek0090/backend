<?php
    ...
    if($technical == 1)
    {
        ...
        $old_qualification = $get_details_row['qualification'];

        $insert_into_update_table = "INSERT INTO `freelancer_update` (`freelancer_id`, `field`, `value`, `updated_on`) VALUES ('$freelancer_id', 'technical_qualification', '$old_qualification', '$now')";
        ...
        {
            $update_sql = "UPDATE `freelancers_technical` SET `qualification` = '$qualification' WHERE `freelancer_id` = '$freelancer_id'";
    ...
    elseif($get_domain_row['non technical'] == 1)
    {
        ...
        $insert_into_update_table = "INSERT INTO `freelancer_update` (`freelancer_id`, `field`, `value`, `updated_on`) VALUES ('$freelancer_id', 'nontechnical_qualification', '$old_qualification', '$now')";
        $insert_into_update_table_result = mysqli_query($conn, $insert_into_update_table) or die(mysqli_error($conn));

        if($insert_into_update_table_result == true)
        {
            $update_sql = "UPDATE `freelancers_nontechnical` SET `qualification` = '$qualification' WHERE `freelancer_id` = '$freelancer_id'";
            $update_result = mysqli_query($conn, $update_sql) or die(mysqli_error($conn));
