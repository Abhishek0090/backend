<?php
    ...
    // if data not entered in map table, don't move files, also give status in each response along with display message
    ...
    if($submit === "submit")
    {
        ...
        if($freelancer_role == "Technical")
        {
            ...
            {
                // write update query
                $data_in_map_sql = "UPDATE `freelancers_map` SET `technical` = 1, `technical_form_filled` = 1 WHERE `email` = '$freelancer_email'";
                $data_in_map = mysqli_query($conn, $data_in_map_sql);
            }
            else
            {
                // write insert query
                $data_in_map_sql = "INSERT INTO `freelancers_map`(`email`, `email_verified`, `country_name`, `country_code`, `number`, `number_verified`, `technical`, `non technical`, `writer`, `technical_form_filled`, `firstname`, `lastname`, `gender`, `whatsapp`, `address`, `street`, `city`, `state`, `pincode`, `agreed`, `non_technical_form_filled`, `writer_form_filled`) 
                VALUES ('$freelancer_email', '$freelancer_email_verified', '$freelancer_country_name', '$freelancer_country_code', '$freelancer_number', '$freelancer_number_verified', 1, 0, 0, 1, '$freelancer_firstname', '$freelancer_lastname', '$freelancer_gender', '$freelancer_whatsapp', '$freelancer_address', '$freelancer_street', '$freelancer_city', '$freelancer_state', '$freelancer_pincode', 1, 0, 0)";
                $data_in_map = mysqli_query($conn, $data_in_map_sql);
            }
            ...
            if($data_in_map == true)
            {
                ...
                $insert_into_technical_freelancers_sql = "INSERT INTO 
                `freelancers_technical`(`freelancer_id`, `domains`, `assignment_type`, `qualification`, `working_hours`, `subject_tags`, `past_experience`, `work_links`, `linkedin`, `experience`, `past_work_files`, `resume`, `profile_photo`, `date_of_submission`) 
                VALUES ('$freelancer_id', '$domains', '$assignment_type', '$qualification', '$working_hours', '$subject_tags', '$past_experience', '$work_links', '$linkedin', '$experience', '$past_work_files_string', '$resume_files_string', '$profile_photo_files_string', '$now')";
                $insert_into_technical_freelancers = mysqli_query($conn, $insert_into_technical_freelancers_sql);
    
                if($insert_into_technical_freelancers == true)
                {
                ...
                else if($insert_into_technical_freelancers == false)
                {
                    echo json_encode(array(
