...
// else show invalid link
    ...
    $check_email_in_table_sql = "SELECT * FROM `freelancers_map` WHERE `email` = '$email_id' ORDER BY `id` DESC LIMIT 1";
    ...
    $freelancer_firstname = $check_email_in_table_row['firstname'];
    $freelancer_lastname = $check_email_in_table_row['lastname'];
    ...
    if($check_email_in_table_rows > 0)
    {
        ...
        $et1 = strtotime($et);
        $link = "bluepen.co.in/auth/changepasswordfreelancer?et=".$et1."&item=".urlencode(base64_encode($id));

        $now = date("Y-m-d H:i:s", strtotime("now"));
        $minutes = date("Y-m-d H:i:s", strtotime("now + 60 minutes"));
        $time = strtotime($minutes);

        mailer1($email_id, $freelancer_firstname, $freelancer_lastname, $link);
        exit();
    ...
    else
    {
        $response = array(
            'status' => 'error', 
            'message' => 'Please register as a freelancer'
        );
        echo json_encode($response);
    }
