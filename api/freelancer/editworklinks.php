<?php
    ...
    if($technical == 1)
    {
        ...
        $old_work_links = $get_details_row['work_links'];

        $insert_into_update_table = "INSERT INTO `freelancer_update` (`freelancer_id`, `field`, `value`, `updated_on`) VALUES ('$freelancer_id', 'technical_work_links', '$old_work_links', '$now')";
        ...
        {
            $update_sql = "UPDATE `freelancers_technical` SET `work_links` = '$work_links' WHERE `freelancer_id` = '$freelancer_id'";
            $update_result = mysqli_query($conn, $update_sql) or die(mysqli_error($conn));

            if($update_result == true)
            {
                $status = 200;
                $message = "Work Links Updated Successfully";
            }
        }
    }
    ...
    elseif($get_domain_row['non technical'] == 1)
    {
        ...
        $insert_into_update_table = "INSERT INTO `freelancer_update` (`freelancer_id`, `field`, `value`, `updated_on`) VALUES ('$freelancer_id', 'nontechnical_work_links', '$old_work_links', '$now')";
        $insert_into_update_table_result = mysqli_query($conn, $insert_into_update_table) or die(mysqli_error($conn));

        if($insert_into_update_table_result == true)
        {
            $update_sql = "UPDATE `freelancers_nontechnical` SET `work_links` = '$work_links' WHERE `freelancer_id` = '$freelancer_id'";
            $update_result = mysqli_query($conn, $update_sql) or die(mysqli_error($conn));
