<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $writing_sample=$_FILES['writing_sample']['name'];

        $error = $_FILES['writing_sample']['error'];
        
        $FileType = strtolower(pathinfo($writing_sample,PATHINFO_EXTENSION));

        for($j=0;$j<count($writing_sample);$j++)
        {
            $FileType = strtolower(pathinfo($writing_sample[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$writing_sample[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($writing_sample);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['writing_sample']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $profiletargetadmin = "../../files/uploads/writers/writing_sample/".$random_number.$_FILES['writing_sample']['name'][$i];
        ...
        VALUES ('writer_writing_sample', '$arr', '$now')";
