<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        $diagram_sample=$_FILES['diagram_sample']['name'];

        $error = $_FILES['diagram_sample']['error'];
        
        $FileType = strtolower(pathinfo($diagram_sample,PATHINFO_EXTENSION));

        for($j=0;$j<count($diagram_sample);$j++)
        {
            $FileType = strtolower(pathinfo($diagram_sample[$j],PATHINFO_EXTENSION));
            $arr = $random_number.$diagram_sample[$j].'_$_'.$arr;
        }
        
        $flag = 0;
        $profileimage = count($diagram_sample);
        $profiletarget2='';
        $amount=0;

        for( $i=0 ; $i < $profileimage ; $i++ )
        {
            $profiletarget = $_FILES['diagram_sample']['tmp_name'][$i];
            $profiletarget2=$profiletarget2.','.$profiletarget;
            if ($profiletarget != "")
            {
                $profiletargetadmin = "../../files/uploads/writers/diagram_sample/".$random_number.$_FILES['diagram_sample']['name'][$i];
        ...
        VALUES ('writer_diagram_sample', '$arr', '$now')";
