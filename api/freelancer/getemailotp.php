<?php
    ...
    else
    {
        ...
        $insert_otp_sql = "INSERT INTO `freelancer_otp_email` (`email`, `otp`, `category`, `generation`, `expiry`) VALUES ('$email', '$otp', '$category', '$now', '$minutes')";
        $insert_otp_result = mysqli_query($conn, $insert_otp_sql) or die(mysqli_error($conn));

        $get_otp_sql = "SELECT * FROM `freelancer_otp_email` WHERE `email` = '$email' ORDER BY `id` DESC LIMIT 1";
